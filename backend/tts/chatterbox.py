...
MP3_DIR = "/tmp/kokito"
SERVIDOR_CHATTERBOX = os.getenv("CHATTERBOX_URL", "http://192.168.1.51:8002")

# Parámetros de Chatterbox Multilingual v3 — equivalentes a temperature/repetition_penalty de XTTS v2.
# exaggeration: 0-1, cuánto exagera la prosodia/emoción (0.5 = neutro, como referencia)
# cfg_weight: 0-1, cuánto se ciñe al audio de referencia (más bajo = más libre/expresivo)
...
def process_file_with_chatterbox(self, pdf_bytes, filename, pagina_inicio=0, pagina_fin=None,
    ...
    # Reutiliza el preprocesado de Coqui como punto de partida — Chatterbox
    # es zero-shot como XTTS, así que probablemente necesite fragmentación
    # similar. Pendiente de ajustar tras las pruebas de calidad en el sobremesa.
    ...
    db = SessionLocal()
    conversion = Conversion(nombre=filename, caracteres=len(text), proveedor="chatterbox")
    db.add(conversion)
    db.commit()
    db.close()
